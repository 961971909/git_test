...
void RN8032_Updata(void)
{
    ...
    float Hutemp,Futemp,CTtmp,PTtmp;

    CTtmp = (float)g_tParam.CtrlParam.CT1Num*1000/Point_Num[g_tParam.CtrlParam.CT_Point]/g_tParam.CtrlParam.CT2Num;
    PTtmp = (float)g_tParam.CtrlParam.PT1Num*1000/Point_Num[g_tParam.CtrlParam.PT_Point]/g_tParam.CtrlParam.PT2Num;
    /*--------------------基波数据--------------------*/
    ...
    for (ph = 0; ph < 3; ph++)
    {
        ...
        if(RN8302RD(&Fullwave_data.U[ph]))
        {
            ...
            if (vg_Power_Val.Real2_Val[ph].U < 200) // 1V    22000 = 220V
            {
                vg_Power_Val.Real2_Val[ph].U = 0;
                vg_Power2_Val.Real22_Val[ph].U = 0;
                vg_Read_Val.Real_Read_Val[ph].U = 0;
            }
            vg_Power_Val.Real2_Val[ph].U = vg_Power_Val.Real2_Val[ph].U * PTtmp;
        ...
        if(RN8302RD(&Fullwave_data.I[ph]))
        {
            ...
            if (vg_Power_Val.Real2_Val[ph].I < 100)  //10ma
            {
                vg_Power_Val.Real2_Val[ph].I = 0;
                vg_Power2_Val.Real22_Val[ph].I = 0;
                vg_Read_Val.Real_Read_Val[ph].I = 0;
            }
            vg_Power_Val.Real2_Val[ph].I = vg_Power_Val.Real2_Val[ph].I * CTtmp;
        ...
        if(RN8302RD(&Fullwave_data.P[ph]))
        {
            if(Fullwave_data.P[ph].data & 0x80000000)
            {
                ...
                vg_Power_Val.Real2_Val[ph].P = ftemp * CTtmp * PTtmp;
        ...
        if(RN8302RD(&Fullwave_data.Q[ph]))
        {
            ...
            {
                ntemp = (((Fullwave_data.Q[ph].data ^ 0xffffffff) + 1) & 0x7fffffff);            // A相无功功率正
                ftemp = (float)ntemp * RATIED_VAL_P / STANDARD_VAL_P;
                if(ftemp < 0.05)
                {
                    ftemp = 0;
                ...
                vg_Power_Val.Real2_Val[ph].Q = ftemp * CTtmp * PTtmp;
        ...
        if(RN8302RD(&Fullwave_data.S[ph]))
        {
            ...
            vg_Power_Val.Real2_Val[ph].S = ftemp * CTtmp * PTtmp;
        ...
        else
        {
            if(RN8302RD(&Fundwave_data.HU[ph]))  //谐波电压有效值
            {
                ...
                ftemp2 = ftemp * RATIED_VAL_U / STANDARD_VAL_U;       // ABC相电压有效值
                vg_Power_Val.Real2_Val[ph].UH = ftemp2 * PTtmp;
        ...
        else
        {
            if(RN8302RD(&Fundwave_data.HI[ph]))
            {
                ...
                ftemp2 = ftemp * RATIED_VAL_I / STANDARD_VAL_I;         // ABC相电流有效值
                vg_Power_Val.Real2_Val[ph].IH = ftemp2 * CTtmp;
	...
	vg_Power2_Val.Real22_Val[0].LineU = (int)(vg_Power2_Val.Real22_Val[0].LineU /PTtmp);
    vg_Power2_Val.Real22_Val[1].LineU = (int)(vg_Power2_Val.Real22_Val[1].LineU /PTtmp);
    vg_Power2_Val.Real22_Val[2].LineU = (int)(vg_Power2_Val.Real22_Val[2].LineU /PTtmp);
    ...
    if((g_tParam.CtrlParam.NetMode == N33) && (vg_Power_Val.Real2_Val[PH_B].I < 50))
    {
        ...
        vg_Read_Val.Real_Read_Val[PH_B].I = (u32)(vg_Power_Val.Real2_Val[PH_B].I/CTtmp);
        vg_Power2_Val.Real22_Val[PH_B].I = (int)(vg_Power_Val.Real2_Val[PH_B].I/10);
        vg_Power2_Val.Real22_Val[PH_B].I = (int)(vg_Power2_Val.Real22_Val[PH_B].I/CTtmp);
        g_HarmIEnable = TRUE;
    }
    else
    {
    ...
    /*------ 电压矢量和有效值 ------*/
    if(RN8302RD(&Fullwave_data.USum_vector))
    {
        ftemp = Fullwave_data.USum_vector.data * 4;
        ftemp2 = ftemp * RATIED_VAL_U / STANDARD_VAL_U;       // ABC相电压有效值
        vg_Power_Val.Usum = ftemp2;
        vg_Power2_Val.Usum = (int)(ftemp2/10);

        if (vg_Power_Val.Usum < 100) // 1V    22000 = 220V
        {
            vg_Power_Val.Usum = 0;
            vg_Power2_Val.Usum = 0;
        }
        vg_Power_Val.Usum = vg_Power_Val.Usum* PTtmp;

        RtuPrimaryData[Index_1_Usum] = vg_Power_Val.Usum/100;
        RtuSecondaryData[Index_2_Usum] = vg_Power2_Val.Usum;
    }
    /*------ 电流矢量和有效值 ------*/
    if(RN8302RD(&Fullwave_data.ISum_vector))
    {
        ftemp = Fullwave_data.ISum_vector.data * 4;
        ftemp2 = ftemp * RATIED_VAL_I / STANDARD_VAL_I;         // ABC相电流有效值
        vg_Power_Val.Isum = ftemp2;
        vg_Power2_Val.Isum = (int)(ftemp2/10);
        if (vg_Power_Val.Isum < 200)  //20ma
        {
            vg_Power_Val.Isum = 0;
            vg_Power2_Val.Isum = 0;
        }
        vg_Power_Val.Isum = vg_Power_Val.Isum * CTtmp;

        RtuPrimaryData[Index_1_Isum] = vg_Power_Val.Isum/10000;
        RtuSecondaryData[Index_2_Isum] = vg_Power2_Val.Isum;
    }
    ...
    {
        Umax = (vg_Power_Val.Real2_Val[0].LineU > vg_Power_Val.Real2_Val[1].LineU)?vg_Power_Val.Real2_Val[0].LineU:
            vg_Power_Val.Real2_Val[1].LineU;
        Umax = (Umax > vg_Power_Val.Real2_Val[2].LineU)? Umax:vg_Power_Val.Real2_Val[2].LineU;

        Umin = (vg_Power_Val.Real2_Val[0].LineU< vg_Power_Val.Real2_Val[1].LineU)?vg_Power_Val.Real2_Val[0].LineU:
            vg_Power_Val.Real2_Val[1].LineU;
    ...
    if(RN8302RD(&Fullwave_data.Pt))
    {
        if(Fullwave_data.Pt.data & 0x80000000)
        {
            ...
            vg_Power_Val.Pt = ftemp * CTtmp * PTtmp;
    ...
    if(RN8302RD(&Fullwave_data.Qt))
    {
        if(Fullwave_data.Qt.data & 0x80000000)
        {
            ...
            vg_Power_Val.Qt = ftemp * CTtmp * PTtmp;
    ...
    if(RN8302RD(&Fullwave_data.St))
    {
        if(Fullwave_data.St.data & 0x80000000)
        {
            ...
            vg_Power_Val.St = ftemp * CTtmp * PTtmp;
    ...
    if(RN8302RD(&CFG_StatusREG.PQSign))
    {
        ...
        if(g_tParam.CtrlParam.NetMode == N33)
        {
            ...
            vg_Power2_Val.St = vg_Power_Val.St/ CTtmp / PTtmp /10;
    ...
    RtuPrimaryData[Index_1_P] = vg_Power_Val.Pt/100000;
    RtuSecondaryData[Index_2_P] = vg_Power2_Val.Pt/10;
    RtuPrimaryData[Index_1_Q] = vg_Power_Val.Qt/100000;
    RtuSecondaryData[Index_2_Q] = vg_Power2_Val.Qt/10;
    RtuPrimaryData[Index_1_S] = vg_Power_Val.St/100000;
    RtuSecondaryData[Index_2_S] = vg_Power2_Val.St/10;
    ...
    if(EnCont > 20)
    {
        ...
        {
            ElectricEnergy.PosEPT += ((float)Fullwave_data.PosEPt.data)/3200 * CTtmp * PTtmp;
            vg_Power2_Val.PosEPT = (int)(ElectricEnergy.PosEPT*1000);
        }
        if(RN8302RD(&Fullwave_data.PosEQt))   // 正向无功电能
        {
            ElectricEnergy.PosEQT += ((float)Fullwave_data.PosEQt.data)/3200 * CTtmp * PTtmp;
            vg_Power2_Val.PosEQT = (int)(ElectricEnergy.PosEQT*1000);
        }
        if(RN8302RD(&Fullwave_data.NegEPt))   // 反向有功电能
        {
            ElectricEnergy.NegEPT += ((float)Fullwave_data.NegEPt.data/3200) * CTtmp * PTtmp;
            vg_Power2_Val.EPT = (int)(ElectricEnergy.NegEPT * 1000);
        }
        if(RN8302RD(&Fullwave_data.NegEQt))   // 反向无功电能
        {
            ElectricEnergy.NegEQT += ((float)Fullwave_data.NegEQt.data/3200) * CTtmp * PTtmp;
...
/* N33与N34工作模式切换 */
void ChangeNetMode(unsigned char choose)
{
    if(choose == 0x00)
    {
        if(g_tParam.CtrlParam.NetMode != N33)
        {
            g_tParam.CtrlParam.NetMode = N33;
            SaveParam();
            DispMode = ReadMode;
            PT_TimerDelay(1000);
            NVIC_SystemReset();
...
//            RN8032_Adjust_Section_I(*pAdjustData);  // 这段删掉
            /* 电流分段1与电流分段2校准修改为N33与N34切换 */
            ChangeNetMode(*pAdjustData);
            break;
        case CALIB_AnoPwm:
            Size = sizeof(AoData);
...
void SampleDataModifyF(float *piRetValue)
{
	...
	u32	Temp;
   	for(i=0; i<NUM_FFT; i++)
   	{
	   	...
	   	Temp = Harmonictemp.InstantaneousData[i*3] <<16 | Harmonictemp.InstantaneousData[i*3+1] <<8 | Harmonictemp.InstantaneousData[i*3+2];
   		if(Temp &0x800000)
   		{
...
/* 谐波电流 */
void HarmonicCurrent()
{
    u8 i;
    for(i = 2; i < 32; i++)
    {
        HarmonicData.HarmonicCurrent[0][i] = HarmonicData.HarmonicpercentI[0][i] * vg_Power_Val.Real2_Val[0].FI / 1000000;
//        HarmonicData.HarmonicCurrent[2][i] = 1123456789 / 1000000;
        HarmonicData.HarmonicCurrent[1][i] = HarmonicData.HarmonicpercentI[1][i] * vg_Power_Val.Real2_Val[1].FI / 1000000;
        HarmonicData.HarmonicCurrent[2][i] = HarmonicData.HarmonicpercentI[2][i] * vg_Power_Val.Real2_Val[2].FI / 1000000;
    }

    for(i = 1; i < 31; i++)
    {
        RtuPrimaryDataHarmI[(i - 1) * 3] = HarmonicData.HarmonicCurrent[0][i] / 10000;
        RtuPrimaryDataHarmI[(i - 1) * 3 + 1] = HarmonicData.HarmonicCurrent[1][i] / 10000;
        RtuPrimaryDataHarmI[(i - 1) * 3 + 2] = HarmonicData.HarmonicCurrent[2][i] / 10000;
    }
}
